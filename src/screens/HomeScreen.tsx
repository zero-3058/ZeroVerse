...
export function HomeScreen() {
  ...
  const [gameEnded, setGameEnded] = useState(false); // prevent double calls
  ...
  const saveTransactionToDB = async (tx: Transaction) => {
    try {
      const response = await fetch("/api/addTransaction", {
        ...
        body: JSON.stringify({ ...tx, tg_id: user?.tg_id }),
      });
  ...
  /** -----------------------------------------
   * GAME OVER → Only send earned points
   * ----------------------------------------- */
  const handleGameOver = async (score: number) => {
    ...
    setTimeout(() => setGameEnded(false), 500);
    setActiveGame(null);
  };

  /** -----------------------------------------
   * TASK COMPLETE → Only send extra reward
   * ----------------------------------------- */
  const handleCompleteTask = async (taskId: string) => {
    ...
    // FIXED: only send reward amount
    await updateUserPoints(task.reward);
  ...
  const handleBackFromGame = () => setActiveGame(null);
